...
```ts
const StringBranded = String.withBrand(
  'my brand',
)
type StringBrandedStatic =  Static<typeof StringBranded>

type Prop = { [K in StringBrandedStatic]: 'prop' }[
  // `Type 'string & { [RuntypeName]: "my brand"; }' cannot be used as an index type.`
  StringBrandedStatic
]
```
